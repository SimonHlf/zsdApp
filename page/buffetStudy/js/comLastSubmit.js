...
function lastSubmitAnswer(currentLoreId,loreTypeName){
	...
	if(lastCommitNumber == questionLength){
		$('.maskLayer').show();
		if(currPageType == 'zhenduanPage_buff'){
			if(loreTypeName == "再次诊断"){
				...
				if(currentAllQuestionFlag == 1){//表示当前题型全部正确
					...
					if(checkFirstStepLore(currentLoreId)){//当第一级关联知识点一次完全通过时
						updateLogStatus(4,0,1,1,currentLoreId,loreTypeName);
					}else{
						if(currentLoreId == originLoreId){//本知识典全部正确
							updateLogStatus(0,1,2,1,currentLoreId,loreTypeName);	
			...
			//执行跳转
			var field = {bsdId:globalBsdId,userId:userInfo.userId};
		...
		}else if(currPageType == 'studyPage_buff'){
			step_new = 3;//永远保持为3
...
function updateLogStatus(step,stepComplete,isFinish,access,currentLoreId,loreTypeName){
	...
	var field = {bsdId:globalBsdId,step:step,stepComplete:stepComplete,isFinish:isFinish,access:access,type:type,currentLoreId:currentLoreId};
	...
	$.ajax({
		...
		dataType:"json",
		data:field,
		url:http_ + "/buffetStudy.do?action=updateLogStatus",
