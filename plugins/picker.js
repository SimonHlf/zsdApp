...
(function(mui, doc) {
	...
	mui.ready(function() {
		...
		//选择学段 网络导师
		app.getId('selSchPicker_nt').addEventListener('tap',function(){
			var schTypeInp_nt = app.getId('schTypeInp_nt'),selSchTypeTxt_nt = app.getId('selSchTypeTxt_nt');
			schTypePicker_nt.show(function(items) {
				selSchTypeTxt_nt.innerText = _getParam(items[0], 'name');
				schTypeInp_nt.value = _getParam(items[0], 'value');
				app.getId('selSchPicker_nt').className = 'hasSel';
				
				//清空之前已选择科目信息
				if(app.getId('subInp').value != ''){
					app.getId('selSubPicker').className = 'noSel';
					app.getId('subInp').value = '';
					app.getId('selSubTxt').innerText = '请选择科目';
				}
			});
		});
		//选择学科 网络导师
		app.getId('selSubPicker').addEventListener('tap',function(){
			var schTypeInp_nt = app.getId('schTypeInp_nt'),selSubTxt = app.getId('selSubTxt'),subInp=app.getId('subInp');
			if(schTypeInp_nt.value == ''){
				mui.toast('请选择学段');
				return;
			}
			mui.ajax(http_ + '/common.do?action=getSubjectDataBySchType',{   
				dataType:'json',
				data:{schType:schTypeInp_nt.value},
				type:'post',
				timeout:10000,  
				beforeSend : function(){
				...
				success:function(json){   
					...
					if(json.result == 'success'){
						...
						//var res = json.subList.map(o=>{return{value:o.subId,  name:o.subName}});
						var res = createNewMap(keyMap,json.subList);
						//装载对应的科目信息数据
						var subPicker = new mui.PopPicker();
						subPicker.setData(res);
						subPicker.show(function(items) {
							selSubTxt.innerText = _getParam(items[0], 'name');
							subInp.value = _getParam(items[0], 'value');
							app.getId('selSubPicker').className = 'hasSel';
						});
